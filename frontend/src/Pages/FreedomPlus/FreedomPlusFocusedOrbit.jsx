import { ArrowLeft, CheckCircle2, Database, ExternalLink, RefreshCw, Users, X } from 'lucide-react'
...
export default function FreedomPlusFocusedOrbit(props) {
  ...
  return <section className="fp-focused-orbit">
    ...
    <div className="fp-focused-orbit__controls"><label>Level<select value={selectedLevel} onChange={(event) => { setSelectedLevel(Number(event.target.value)); setCycle(''); setSelectedPosition(null) }}>{levels.map((item) => <option key={item.level} value={item.level} disabled={!activeLevels.has(item.level)}>Level {item.level} / {item.orbit}</option>)}</select></label><label>Cycle<select value={cycle} onChange={(event) => { setCycle(event.target.value); setSelectedPosition(null) }}><option value="">Current</option>{cycles.map((item) => <option value={item} key={item}>Cycle {item}</option>)}</select></label><button type="button" onClick={onRefresh} disabled={loading}><RefreshCw className={loading ? 'spin' : ''} />Refresh</button></div>
    ...
    <div className="fp-focused-orbit__canvas"><FreedomPlusOrbit orbitType={config?.orbit} positions={positions} owner={account} onSelect={setSelectedPosition} selectedPosition={selectedPosition} /></div>
