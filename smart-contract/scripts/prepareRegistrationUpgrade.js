...
async function main() {
  ...
  const proxyAddress = process.env.REGISTRATION_ADDRESS || process.env.VITE_REGISTRATION_ADDRESS;
  if (!proxyAddress || !ethers.isAddress(proxyAddress)) {
    throw new Error("REGISTRATION_ADDRESS is required and must be a valid proxy address");
  }

  console.log("Preparing RegistrationFixed implementation upgrade");
  console.log("Network:", hre.network.name);
  console.log("Deployer:", deployer.address);
  console.log("Proxy:", proxyAddress);

  const factory = await ethers.getContractFactory("RegistrationFixed");
